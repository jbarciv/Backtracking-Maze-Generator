...
Es importante resaltar en este punto, lo conveniente que ha sido implementar en ambos código el paso de parámetros a la función \textsc{main()}. Se incluye brevemente el código y las llamadas a las funciones que reciben dichos argumentos:
\lstset{style=mystyle}
%también se puede poner [firstnumber=39]
\begin{lstlisting}[language=C++, title= Variente para la función \textsf{PrintGrid()} con carácteres \textsc{unicode}, frame=single, numbers=none]
	//----FUNCION PRINCIPAL-------------------------------------
